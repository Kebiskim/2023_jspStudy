...
<%--
	response(응답) : 응답에 대한 정보를 가지고 있음
	  ...
	  2. JS를 통해서 웹 페이지 이동
	  	 ...
	  	 - history.go(숫자) : 웹 브라우저의 앞으로, 뒤로와 같은 효과를 낸다.
	  
	  ** 1과 2는 이벤트 처리를 해야만 웹 페이지가 이동된다. (즉, 현재 페이지가 보인다.)
	  ...
	  4. 포워드	: request.getRequestDispatcher("이동할주소").forward(request, response);
...
<p><a href="Ex05_Result.jsp?name=둘리&age=24&gender=남&hobby=운동&hobby=게임">Ex05_Result.jsp(파라미터 없음)</a></p>
<hr>
